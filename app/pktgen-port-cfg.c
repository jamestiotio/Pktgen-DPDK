...
#if 0
static void
pktgen_tx_retry_callback(struct rte_mbuf **pkts, uint16_t unsent, void *userdata)
{
    struct eth_tx_buffer *txbuff = userdata;
    
    printf(">>>> %d-%d length %6d, unsent %4d  %p\n", txbuff->pid, txbuff->qid, txbuff->length, unsent, txbuff);
    ...
    printf("<<<< %d-%d length %6d\n", txbuff->pid, txbuff->qid, txbuff->length);
}
#endif
