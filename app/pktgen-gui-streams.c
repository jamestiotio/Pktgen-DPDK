...
fill_proto_field_info(proto_type type, unsigned int pid, unsigned int seq_id)
{
    ...
    if (type == TYPE_ETH) {
        struct rte_ether_addr *eaddr = &pkt->eth_dst_addr;
        val.name                     = g_strdup(pktgen_ethernet_fields[i++]);
        snprintf(buff, sizeof(buff), "%02x%02x%02x%02x%02x%02x", eaddr->addr_bytes[0],
                 eaddr->addr_bytes[1], eaddr->addr_bytes[2], eaddr->addr_bytes[3],
        ...
        size = (pkt->pktSize + RTE_ETHER_CRC_LEN);
        sprintf(buff, "%d", size);
        ...
        val.name  = g_strdup(pktgen_ipv4_fields[i++]);
        val.value = g_strdup((pkt->ipProto == PG_IPPROTO_UDP) ? "UDP" : "User Defined");
        g_array_append_vals(packet_info, &val, 1);
        ...
        val.name  = g_strdup(pktgen_ipv4_fields[i++]);
        val.value = g_strdup(
            inet_ntop4(buff, sizeof(buff), ntohl(pkt->ip_src_addr.addr.ipv4.s_addr), 0xFFFFFFFF));
        g_array_append_vals(packet_info, &val, 1);

        val.name  = g_strdup(pktgen_ipv4_fields[i++]);
        val.value = g_strdup(
            inet_ntop4(buff, sizeof(buff), ntohl(pkt->ip_dst_addr.addr.ipv4.s_addr), 0xFFFFFFFF));
        g_array_append_vals(packet_info, &val, 1);
...
create_stream_model(proto_type type, unsigned int pid, unsigned int seq_id)
{
    ...
    for (; i < packet_info->len; i++) {
        ...
        gtk_list_store_set(model, &iter, COLUMN_NAME, g_array_index(packet_info, protocol, i).name,
                           COLUMN_VALUE, g_array_index(packet_info, protocol, i).value,
...
void
cell_edited_callback(GtkCellRendererText *cell, const gchar *path_string, gchar *new_text,
                     gpointer data)
{
    ...
    case COLUMN_VALUE: {
        ...
        if (0 == strcmp(col_name, pktgen_ethernet_fields[3]))
            if ((atoi(new_text) > 4096) || (atoi(new_text) < 1)) {
                ...
                dialog = gtk_message_dialog_new(
                    GTK_WINDOW(stream_window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
                    GTK_BUTTONS_OK, "Invalid VLAN Id! It should be [1-4095]");
        ...
        if ((0 == strcmp(col_name, pktgen_ipv4_fields[9])) ||
            (0 == strcmp(col_name, pktgen_ipv4_fields[10])))
            if (!validate_ip_address(new_ip)) {
                GtkWidget *dialog;
                dialog = gtk_message_dialog_new(
                    GTK_WINDOW(stream_window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
                    GTK_BUTTONS_OK, "The given IP is not a valid IP address!");
                gtk_window_set_title(GTK_WINDOW(dialog), "Pktgen");
                gtk_dialog_run(GTK_DIALOG(dialog));
                gtk_widget_destroy(dialog);
                return;
        ...
        if ((0 == strcmp(col_name, pktgen_ethernet_fields[0])) ||
            ...
            if (12 == strlen(new_text)) {
                ...
                for (i = 0; i < strlen(new_text); i++) {
                    ...
                    if ((*ch >= 'a' && *ch <= 'f') || (*ch >= 'A' && *ch <= 'F') ||
                        ...
                        GtkWidget *dialog;
                        dialog = gtk_message_dialog_new(GTK_WINDOW(stream_window),
                                                        GTK_DIALOG_DESTROY_WITH_PARENT,
                                                        GTK_MESSAGE_INFO, GTK_BUTTONS_OK,
                                                        "Please input only hex values[0-9][a-f], "
        ...
        if ((0 != strcmp(old_text, "<auto>")) && (0 != strcmp(old_text, "IPv4")) &&
            (0 != strcmp(old_text, "UDP"))) {
            g_free(old_text);
            ...
            g_free(g_array_index(packet_info, protocol, (i + index_offset)).value);
            g_array_index(packet_info, protocol, (i + index_offset)).value = g_strdup(new_text);

            gtk_list_store_set(GTK_LIST_STORE(model), &iter, column,
                               g_array_index(packet_info, protocol, (i + index_offset)).value, -1);
        } else {
            GtkWidget *dialog;
            dialog = gtk_message_dialog_new(
                GTK_WINDOW(stream_window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
                GTK_BUTTONS_OK, "Auto generated values can not be modified!");
            gtk_window_set_title(GTK_WINDOW(dialog), "Pktgen");
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
        }
...
void
add_proto_values_column(GtkTreeView *treeview, GtkTreeModel *stream_model)
{
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(treeview), -1, "Name", renderer,
                                                "text", COLUMN_NAME, NULL);

    /* value column */
    renderer = gtk_cell_renderer_text_new();
    g_object_set(renderer, "editable", TRUE, NULL);
    g_signal_connect(renderer, "edited", G_CALLBACK(cell_edited_callback), stream_model);
    g_object_set_data(G_OBJECT(renderer), "column", GINT_TO_POINTER(COLUMN_VALUE));
...
fill_stream_info(proto_type type, unsigned int pid, unsigned int seq_id)
{
    ...
    gtk_box_pack_start(GTK_BOX(vbox), gtk_label_new("Edit Protocol fields"), FALSE, FALSE, 0);
    ...
    g_signal_connect(next_button, "clicked", G_CALLBACK(switch_stream_editor_page), notebook);

    gtk_box_pack_start(GTK_BOX(hbox), next_button, TRUE, TRUE, 0);

    apply_button = gtk_button_new_with_label("Apply");
    g_signal_connect(apply_button, "clicked", G_CALLBACK(apply_stream_callback), NULL);

    gtk_box_pack_start(GTK_BOX(hbox), apply_button, TRUE, TRUE, 0);

    quit_button = gtk_button_new_with_label("Close");

    g_signal_connect(quit_button, "clicked", G_CALLBACK(close_window_callback),
...
pktsize_enter_callback(GtkWidget *widget, gpointer *data)
{
    ...
    if (((size - RTE_ETHER_CRC_LEN) < (unsigned int)MIN_PKT_SIZE) ||
        ((size - RTE_ETHER_CRC_LEN) > (unsigned int)MAX_PKT_SIZE)) {
        GtkWidget *dialog;
        dialog = gtk_message_dialog_new(
            GTK_WINDOW(stream_window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
            GTK_BUTTONS_OK,
            "Acceptable range is [%d - %d]\nAlphabets/special characters are not allowed",
            (MIN_PKT_SIZE + RTE_ETHER_CRC_LEN), (MAX_PKT_SIZE + RTE_ETHER_CRC_LEN));
        gtk_window_set_title(GTK_WINDOW(dialog), "Pktgen");
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
...
radio_options_callback(GtkRadioButton *rb, gpointer *user_data)
{
    ...
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(rb))) {
        if (l4_text_view->parent != GTK_WIDGET(udp_sw)) {
            ...
            gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(udp_sw), GTK_POLICY_NEVER,
                                           GTK_POLICY_NEVER);
            ...
            pkt->ipProto =
                PG_IPPROTO_USR_DEF; /* Set this value to bypass ctor to fill l4 protocols */
        }
    } else {
        sprintf(buff, "%d", PG_IPPROTO_UDP);
        gtk_entry_set_text(GTK_ENTRY(ip_proto_entry), buff);
        gtk_entry_set_editable(GTK_ENTRY(ip_proto_entry), FALSE);
        gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(udp_sw), GTK_POLICY_AUTOMATIC,
                                       GTK_POLICY_AUTOMATIC);
...
display_stream_editor(GtkWidget *window, unsigned int pid, unsigned int seq_id)
{
    ...
    packet_info = g_array_sized_new(FALSE, FALSE, sizeof(protocol), (1 + 3 + 11 + 4));
    ...
    gtk_signal_connect(GTK_OBJECT(stream_l2_vlan), "toggled", GTK_SIGNAL_FUNC(vlan_enable_callback),
                       (gpointer)&pid);
    ...
    GtkWidget *radio_button_usrdef = gtk_radio_button_new_with_label_from_widget(
        GTK_RADIO_BUTTON(radio_button_udp), "User Defined");
    ...
    g_signal_connect(G_OBJECT(radio_button_usrdef), "clicked", G_CALLBACK(radio_options_callback),
                     (gpointer)&pid);

    GtkWidget *hbox2 = gtk_hbox_new(FALSE, 0);
    gtk_box_pack_start(GTK_BOX(hbox2), gtk_label_new(NULL), TRUE, TRUE,
    ...
    gtk_box_pack_start(GTK_BOX(stream_l2_vbox), fill_stream_info(TYPE_ETH, pid, seq_id), TRUE, TRUE,
                       0);

    stream_l3_vbox = gtk_vbox_new(FALSE, 0);
    gtk_box_pack_start(GTK_BOX(stream_l3_vbox), fill_stream_info(TYPE_IPv4, pid, seq_id), TRUE,
                       TRUE, 0);

    stream_l4_vbox = gtk_vbox_new(FALSE, 0);
    gtk_box_pack_start(GTK_BOX(stream_l4_vbox), fill_stream_info(TYPE_UDP, pid, seq_id), TRUE, TRUE,
                       0);
...
void
switch_stream_editor_page(GtkButton __attribute__((unused)) * button, GtkNotebook *notebook)
{
...
apply_stream_callback(void)
{
    ...
    gtk_tree_selection_selected_foreach(selection_stream, traffic_stream_get_seq_id,
                                        (gpointer)&seq_id);
...
set_stream_info(unsigned int pid, unsigned int seq_id)
{
    ...
    unsigned int l4_and_payload[MAX_PKT_SIZE - RTE_ETHER_CRC_LEN];
    gchar usr_def_str[MAX_PKT_SIZE - RTE_ETHER_CRC_LEN];
    GtkTextIter start;
    GtkTextIter end;
    unsigned int size;
    ...
    strcpy(usr_def_str, gtk_text_buffer_get_text(l4_pl_buffer, &start, &end, FALSE));
    ...
    for (i = 0; i < packet_info->len; i++) {
        ...
        case 0: /* Destination MAC */
            strcpy(str, g_array_index(packet_info, protocol, i).value);
        ...
        case 3: /* Vlan ID */
            pkt->vlanid = atoi(g_array_index(packet_info, protocol, i).value);
            break;

        case 13: /* Source IP Address */
            inet_aton(g_array_index(packet_info, protocol, i).value, &new_src_ip.sin_addr);
            pkt->ip_src_addr.addr.ipv4.s_addr = ntohl(new_src_ip.sin_addr.s_addr);
            break;

        case 14: /* Destination IP Address */
            inet_aton(g_array_index(packet_info, protocol, i).value, &new_dst_ip.sin_addr);
            pkt->ip_dst_addr.addr.ipv4.s_addr = ntohl(new_dst_ip.sin_addr.s_addr);
            break;

        case 15: /* Source Port */
            pkt->sport = atoi(g_array_index(packet_info, protocol, i).value);
            break;

        case 16: /* Destination Port */
            pkt->dport = atoi(g_array_index(packet_info, protocol, i).value);
            break;
        }
    }
    ...
    for (i = 0; i < strlen(usr_def_str) / 2; i++)
        usr_def[i + sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr)] = l4_and_payload[i];

    usr_def[sizeof(struct rte_ether_hdr) + 9] =
        ip_proto_value[0]; /* Overwrite the IPv4 protocol field */
...
edit_stream(void)
{
    ...
    g_signal_connect(stream_window, "destroy", G_CALLBACK(gtk_main_quit), NULL);
...

    {
        ...
        if (no_rows == 0) {
            ...
            dialog = gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT,
                                            GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
                                            "Traffic stream editing for multiple ports cannot be "
                                            "done simultaneously!! \nPlease select a single port");
            gtk_window_set_title(GTK_WINDOW(dialog), "Pktgen");
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
        } else
            gtk_tree_selection_selected_foreach(selection, edit_stream_callback,
...
void
edit_stream_callback(GtkTreeModel *model, GtkTreePath __attribute__((unused)) * path,
...
stream_box(void)
{
    ...
    gtk_signal_connect(GTK_OBJECT(apply_button), "clicked", GTK_SIGNAL_FUNC(apply_callback), NULL);

    tx_rate_range = gtk_adjustment_new(1, 1.0, 101.0, 1.0, 1.0, 1.0);
    g_signal_connect(tx_rate_range, "value_changed", G_CALLBACK(digits_scale_callback), NULL);
    ...
    for (pid = 0; pid < RTE_MAX_ETHPORTS; pid++) {
        ...
        for (j = 0; j < NUM_SEQ_PKTS; j++) {
            ...
            str_db[pid][j].stream_name = (char *)malloc(25 * sizeof(char));
            sprintf(stream_str, "%s-%d-%s-%d", "Port", pid, "stream", j);
        ...
        /* Set seqCnt to maximum as enable/disable stream will take care of sending selected stream
         * from GUI */
        ...
        /* create tree model */
        traffic_stream[pid] =
            gtk_tree_store_new(NUM_TRAF_STR_COLS, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_INT);
        ...
        while (port_traffic->path != NULL) {
            ...
            gtk_tree_model_get_iter(GTK_TREE_MODEL(traffic_stream[pid]), &iter, path);
            gtk_tree_path_free(path);
            gtk_tree_store_set(traffic_stream[pid], &iter, TRAF_STR_NAME, port_traffic->stream_name,
                               TRAF_STR_SELECT, port_traffic->stream_select, TRAF_STR_NO,
                               port_traffic->stream_no, -1);
            port_traffic++;
        }
        ...
        gtk_tree_view_append_column(GTK_TREE_VIEW(stream_view[pid]), sl_no_col);
        gtk_tree_view_append_column(GTK_TREE_VIEW(stream_view[pid]), stream_name_col);
        gtk_tree_view_append_column(GTK_TREE_VIEW(stream_view[pid]), select_col);
        ...
        gtk_tree_view_column_pack_start(stream_name_col, name_renderer, TRUE);
        gtk_tree_view_column_pack_start(select_col, check_renderer, TRUE);

        /* connect 'text' property of the cell renderer to
         *  model column that contains the first name */
        gtk_tree_view_column_add_attribute(sl_no_col, num_renderer, "text", TRAF_STR_NO);
        gtk_tree_view_column_add_attribute(stream_name_col, name_renderer, "text", TRAF_STR_NAME);
        gtk_tree_view_column_add_attribute(select_col, check_renderer, "active", TRAF_STR_SELECT);

        gtk_tree_view_set_model(GTK_TREE_VIEW(stream_view[pid]),
                                GTK_TREE_MODEL(traffic_stream[pid]));

        g_signal_connect(G_OBJECT(check_renderer), "toggled", G_CALLBACK(enable_stream_callback),
                         NULL);

        GtkTreeSelection *stream_selection =
            gtk_tree_view_get_selection(GTK_TREE_VIEW(stream_view[pid]));
        gtk_tree_selection_set_mode(stream_selection, GTK_SELECTION_SINGLE);

        g_signal_connect(stream_view[pid], "row-activated", (GCallback)edit_stream, NULL);
    }
