...
coremap(const char *arg)
{
    ...
    printf("%u socket%s, %u core%s per socket and %u thread%s per core\n",
           cnt(lcores, get_socket_id), cnt(lcores, get_socket_id) > 1 ? "s" : "",
           cnt(lcores, get_core_id), cnt(lcores, get_core_id) > 1 ? "s" : "",
           cnt(lcores, get_thread_id), cnt(lcores, get_thread_id) > 1 ? "s" : "");
